...
if os.path.exists(FONT_PATH):
    ...
    fontprop = None
    st.warning("NanumGothic-Bold.ttf 폰트가 필요합니다.")
...
def analyze_spending(spending_data, budget):
    total_spent = sum([x["amount"] for x in spending_data])
    tips = []

    if total_spent > budget:
        tips.append(f"❗ 예산 초과: {total_spent - budget:,}원 초과")
    elif total_spent > budget * 0.9:
        tips.append("⚠️ 예산의 90% 이상 사용 중")
    elif total_spent < budget * 0.5:
...
# ✅ UI 시작
st.set_page_config(page_title="소비 분석 자산 시스템", layout="wide")
...
if col1.button("💾 저장 및 분석"):
    ...
    df_all = df_all[df_all["month"] != month]  # 해당 월 기존 삭제
    ...
    if not pie_df.empty:
        fig, ax = plt.subplots()
        ax.pie(pie_df["amount"], labels=pie_df["category"], autopct="%1.1f%%", startangle=90)
        ax.axis("equal")
        st.pyplot(fig)
